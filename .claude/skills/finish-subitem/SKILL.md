...
### 2. 两套测试都要绿
```bash
.venv/bin/python -m pytest -q                       # 472 passed, 3 skipped
TEST_DATABASE_URL="postgresql+asyncpg://postgres@/codemax_test?host=/tmp/pgdata" \
  .venv/bin/python -m pytest -q                     # 474 passed, 1 skipped
```
...
子项：<S4-01-2 标题>
做了什么：<要点>
测试：SQLite 472 passed + 3 skipped；真 PostgreSQL 16.2 474 passed + 1 skipped
变异测试：<去掉 X → N 个红> ×若干
