...
> 历史阶段快照：下文网络阻断/未执行是当时结果。后续已在 GitHub runner 完成 Agnes 真实聊天/Mermaid 前缀验证，见 [Agnes 阶段](agnes-integration.md)；embedding/FAQ 标定及 Windows/浏览器验收仍未完成。当前任务仅在 ROADMAP 维护。
...
- [x] 后续阶段已完成真实聊天/Mermaid 前缀联调，证据见 Agnes 阶段；不改写本阶段原始未执行记录。
- [ ] 真实 embedding/FAQ 标定仍待选择模型与授权环境；不能把 chat 成功当向量成功。
- [ ] 用户按新手指南完成本机人工项目，填写仓库外证据表；通过/失败/未执行分别保留。

当前收尾结论：管理/工具/指南交付与外部实际验收分开；模型各层与 Windows 实机均分别签收，不因管理交付完成自动勾选。网络受阻没有足够证据归因于用户网络或 GitHub 认证，不再要求无依据的重授权。
