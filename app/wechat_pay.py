"""微信支付 APIv3：NATIVE 扫码下单（S3-01-1）。

只做本项目用得到的事：请求签名、Authorization 头、商户订单号、Native 下单。
回调验签与报文解密属于 S3-01-3，届时补进本模块。
...
两个必须钉死的细节（错了在本地测不出来，只会在真机上验签失败）：
...
能真正验证的是签名算法（自签密钥 + 公钥验签）与 HTTP 请求构造
（`httpx.MockTransport` 断言方法、路径、头、报文），见 `tests/test_wechat_pay.py`。
"""
...
class PayConfig:
    """下单所需的全部凭据。frozen：支付配置不该在请求处理途中被改。"""
