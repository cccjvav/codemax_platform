...
> `database init/README.md`（建表脚本，`models.py` 的对偶）、`app/templates/README.md`（前端模板）、`app/static/README.md`、`.github/workflows/README.md`、`docs/ROOT_FILES.md`、`tests/README.md`、`scripts/README.md`。
...
> - **为什么 CSP 里保留了 `'unsafe-inline'`**（L16-L21）：实测仍有 **5 个模板含内联 `<script>`**（`drawio.html` / `er.html` / `mermaid.html` / `mock_pay.html` / `shop.html`），要上严格 CSP 就得把它们全改成外部文件 + nonce，那是前端重构。
>   **S2-02-2 已经往这个方向走了一步**：全站共享的登录模块做成了外部文件
>   `app/static/js/auth.js`（由 `script-src 'self'` 覆盖，不需要 unsafe-inline），
