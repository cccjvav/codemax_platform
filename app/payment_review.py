...
ISSUES = ('channel_close_started', 'channel_close_acknowledged', 'channel_close_unknown', 'prepay_unknown', 'query_unknown', 'query_conflict', 'query_refund', 'query_aborted', 'refund_query_unknown', 'refund_query_aborted', 'refund_query_conflict',
...
async def review_states(db: AsyncSession, orders: list[Order], *, now: datetime | None = None) -> dict[int, dict]:
    ...
    receipt_rows = (await db.execute(select(PaymentReceipt.order_id, PaymentReceipt.id, RefundReceipt.id, RefundRequest.id, RefundAuthorization.id, RefundSendStop.id)
