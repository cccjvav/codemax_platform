...
def test_pg_upgrade_preserves_history_and_enforces_immutable_evidence(maintenance_db):
    ...
    rows(conn, 'DROP TABLE refund_request; DROP FUNCTION codemax_check_refund_request(); DROP TABLE refund_receipt; DROP FUNCTION codemax_check_full_refund(); DROP TABLE payment_event; DROP TABLE payment_receipt; '
               'DROP FUNCTION codemax_freeze_order_contract() CASCADE; DROP FUNCTION codemax_append_only_evidence() CASCADE; '
               "DELETE FROM schema_migration WHERE version IN ('0010','0011','0012')")
