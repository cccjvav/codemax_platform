...
2. `.venv/bin/python -m pytest -q` → **654 passed, 4 skipped**（4 条 skip 的实测构成见 `README.md`：2 条并发需真库 + 1 条真浏览器 + 1 条语义阈值标定需 embedding key）
...
| **前端 JS 源码目录（改前端先看这里）** | `app/frontend/README.md` |
