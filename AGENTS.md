...
本会话固定分支见 HANDOVER.md 首段的「本会话固定分支」（分支名全仓只维护那一处；指南、Skill 与工作流都不复制这个值）。当前架构见 docs/ARCHITECTURE_GUIDE.md，交接见 HANDOVER.md，内容审核见 review/README.md「最新独立交接」（当前为 review/FULL_REPOSITORY_REVIEW_2026-09-23.md，全仓基线审计为 review/FULL_REPOSITORY_HANDOFF_2026-09-19.md）；历史报告或 Skill 中旧章节号/固定测试数不覆盖当前指南。
...
- 用户已批准的本轮资金/交付迁移、重登录、下载恢复、站内客服与文档重做不反复询问；新的依赖、schema、业务范围或生产操作先确认。
...
- 模型变化同步 models.py、full_init.sql 与必要增量迁移。当前full_init仅允许空库，不用于存量升级；历史版本含DROP，不能回退执行。
- 优先把可复用算法放 tools/公共层；如路由实际包含查询和事务，文档必须如实解释，而非声称已经完全分层。
- 配置集中 Settings；维护CLI与应用共用根目录.env和DATABASE_URL优先级，Compose显式覆盖连接字段，不推广散落的getenv。
