...
2. `.venv/bin/python -m pytest -q` → **472 passed, 3 skipped**（3 条 skip 是真库专用的并发用例）
   （跳过的那条是真并发测试，SQLite 的 StaticPool 复现不了竞态，见 TD-85）。
3. 真 PostgreSQL 上 → **474 passed, 1 skipped**（此前随机红的绝对阈值性能用例已按实测换掉，见 `TECH_DECISIONS.md` TD-183/186）（起库配方见 `HANDOVER.md` §9）。
