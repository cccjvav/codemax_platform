...
**本会话固定分支（全仓唯一维护处，TD-277 / V-08）**：`arena/01a0da70-codemax-platform`（2026-09-25 起；上一会话为 `arena/01a0cdc6-codemax-platform`，本会话从其 tip `69265e1` 分出）。其他现行文档不再复制这个值：Linux 命令用 `$(git branch --show-current)`，Windows 指南用 `set "BRANCH=..."` 从这一行取值，Agnes 工作流按 `arena/*` 前缀触发。
...
**发布状态（2026-09-23）**：`d61098d`（TD-271，复核报告 + 文档同步）已推送到本会话分支，[CI 35869547675](https://github.com/cccjvav/codemax_platform/actions/runs/35869547675) 六项 success（SQLite pytest 19m55s、PG pytest 23m57s）。该节的“认证失效、尚未推送”记录已过期。

**接手状态（2026-09-23 晚，TD-272 批次 1）**：新接手人独立复核后按用户确认施工，落地两项**无新依赖、无新业务规则**的修复：V-03（测试进程 bcrypt 成本 4：全量 1276 s → 219 s，1822 passed；生产成本由 `production_cost` fixture 单独复验）与 V-04/V-06 的一部分（跳过导航、表单控件字体继承、窄屏 16px 输入框与 ≥24px 命中区、顶栏 208px → 147px、工具页可见页面标题、整站每页单 h1、管理员入口按角色显隐、管理页长订单号换行/只读分区/危险动作红框、客服时间对比度 4.34:1 → 6.92:1、Drawio 首屏不再重建 iframe、未配置模型时的访客文案）。修完再用沙箱内真实 Chromium 复测：横向溢出消失、axe-core 全部状态 0 违规、小于 24px 的可点目标归零。**2026-09-23 晚 TD-273（批次 2，P2 回归）**：会话到期与主动退出分开 —— Drawio 到期不重建编辑器、同一账号重登可继续保存，客服草稿保留；换账号仍必须重建（账号隔离不退化）。**2026-09-24 TD-275（V-02，P2）**：`POST/PUT /diagrams` 共用一个 `diagram_write` 写入桶（新配置 `RATE_LIMIT_DIAGRAM_WRITES=60`，每 IP 每 60 秒；读取不挂桶），注册再加每 IP 24 小时上限 `RATE_LIMIT_REGISTER_DAILY=20`（独立桶表与 24 小时窗口 —— 混进主桶表会让过期桶收不回来，F-09 保护退化）。取值按助手建议（用户 2026-09-24 确认）。回归 `tests/test_ratelimit.py` 新增 5 条，在旧路由上全部先红；复核建议里“可选”的全站总字节软上限与运维告警**未做**，如实记录。**2026-09-24 TD-276（V-07，P3）**：`TRUST_PROXY_HEADERS=false` 的生产启动检查保留硬拦、文案改为与行为一致（「必须开启」，并说明是否采信转发头另由 `TRUSTED_PROXY_CIDRS` 决定 —— O-15 由用户委托助手定稿）；gzip 级别 9→6；`/static/*.md` 一律 404（README 文件保留，文档契约要求）；DEPLOY/WINDOWS_LOCAL_RUN 补 N-07 说明；favicon 在 TD-272 已补。回归 `tests/test_ops.py` 新增 3 条，旧代码上先红。**2026-09-24 TD-277（V-08，P3）**：分支名不再硬编码 —— 分支名全仓只在 HANDOVER 首段维护一处；finish-subitem Skill 用 `$(git branch --show-current)`；Windows 指南用 `set "BRANCH=..."` 从 HANDOVER 取值；Agnes 工作流的 push 触发从写死的上一会话分支改为 `arena/*` 前缀（paths 过滤与 `[agnes-live-test]` 门槛不变，测试断言同步改为模式匹配）。**同日提醒**：Agnes 工作流的 live-chat 判定是**整条提交说明的子串匹配**，TD-277 的提交说明引用了该标记，导致 `fee88eb` 上带密钥的四项探测实际执行（聊天/Mermaid 通过、embedding 500）——写提交说明时不要照抄标记；是否把判定收紧成「只匹配首行」**待用户确认**，未确认前不改工作流。**仍未实施**：V-06 余项（网页改密码入口、Mermaid 按需加载）。

**推送状态（2026-09-24 更新）**：上一条“只在本地”的记录已过期 —— 用户重新连接 GitHub 后，`00d4d9b`（TD-272）与 `f366fc3`（TD-273）已推送到本会话分支，`git ls-remote` 核对远端 tip 就是 `f366fc3`。对应的 [CI 35936524079](https://github.com/cccjvav/codemax_platform/actions/runs/35936524079) 在写这段时静态检查、依赖漏洞扫描、文档站构建、前端产物漂移与 SQLite pytest 已 success，真 PostgreSQL 作业仍在跑；**本轮 TD-274 的推送会再触发一次 CI，以推送后的最终 SHA 为准**，不要用 `d61098d` 或中途快照的绿色代替。接手人先 `git log --oneline -5` 与 `git ls-remote` 对照。

**接手状态（2026-09-24，TD-274 / V-05）**：按用户 2026-09-24 的四条答复施工：V-05 落地 —— 已购（paid/downloaded 且未全额退款）用户的主按钮改成「已购买，去下载」，点它回到该单、**不再新建订单**（用户确认单 SKU 不复购）；收银台「返回商城查看订单」链接带 `?order=<订单号>`，`/shop` 加载时据此直接显示该单状态；页面加载/登录后自动恢复一次（已购优先 → 未过期的待支付单接着付 → 过期单不恢复），账号切换清按钮保持隔离；Windows 指南第 12 步文字同步。证据：`tests/test_shop_page.py` 新增 Node 桩组（4 条在旧代码上先红 + 2 条防过修对照）、`tests/test_mock_pay.py` 返回链接断言先红；沙箱内真实 Chromium 复走「下单 → 模拟收银台 → 返回 → 已购按钮 → 手机视图」全流程。**仍未实施**：V-02（流程图写入限流 + 注册上限，P2，取值按助手建议待落地）、V-06 余项（网页改密码入口、Mermaid 按需加载）、V-07/V-08。批次 2 起仍需逐批确认（尤其 V-02/V-08 涉及配置项与工作流合同）。

**接手状态（2026-09-25，TD-278 / V-04 续）**：新会话从 `69265e1` 接手，基线复现：ruff、文档契约、Vite 零漂移、全量 SQLite 1864 passed / 7 skipped。先用真实 Chromium（桌面/手机、axe-core）走遍全部页面与一次模拟购买，再按问题施工：手机顶栏 189px → 88px（两行 grid）；**TD-272 的手机 16px 输入框规则实际被覆盖、从未生效**，已修正并把测试改为钉住层叠前提；付款后「我的订单」挪到状态区之后；Drawio 已登录不再显示「需登录」；订单管理页空状态与选单前占位；单 h1 覆盖收银台/客服/商店。只改模板/前端/样式，无依赖、schema、接口变化。下一批：ER 布局重写（V-04 剩余）；再下一批：改密入口与 Mermaid 按需加载（V-06 剩余）及未读后端模块复核。

**接手状态（2026-09-25 续，TD-279 / V-04 收尾）**：ER 图布局重写，V-04 全部完成。改前用本项目 16 张表实测 20/24 条外键线穿过别的表、长文字压出方框、整图压到约 40% 字号；改后按外键深度分列、正交折线只走通道与车道（穿表 0、溢出 0、axe 零违规），默认 0.75 可读比例并可「查看全图」。只改 `app/frontend/er-*.js`、`er.html`、产物与测试，无依赖/schema/接口变化。下一批：改密入口与 Mermaid 按需加载（V-06 剩余）及未读后端模块复核。

**接手状态（2026-09-25 续，TD-280 / V-06 收尾）**：V-06 全部完成。① 顶栏用户名改为按钮，打开「修改密码」浮层（`POST /auth/password` 早有、此前整站无入口，生产关 `/docs` 后无法自助改密）；真实浏览器实测：其他会话改后 401、本页保持登录，手机顶栏仍 88px、axe 零违规。② Mermaid 按需加载：页面首屏 669 KiB → 3.5 KiB，点「生成类图」才与模型请求并行下载；下载失败不缓存、仍显示源码。③ 顺带修正 Vite `base`：原先动态 import 的预加载地址是 `/mermaid.core.js` 这类 404（功能不受影响、预加载无效）。④ 根 `package.json` 的 `//` 说明改为现行理由。无依赖/schema/接口变化。下一批：未读后端模块复核（shop/oauth/收款与退款管理/限流等）与文档陈旧项。
...
gh run list --workflow ci.yml --branch "$(git branch --show-current)" --limit 5 --json databaseId,headSha,status,conclusion,url
gh run view <匹配HEAD的run-id> --json headSha,status,conclusion,jobs,url
```

本会话固定分支见本页首段「本会话固定分支」；不切分支、不推其他分支。认证诊断依实际仓库/推送结果，不凭 `gh api user` 的权限错误断言 Git 不可用。
