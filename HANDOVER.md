...
- 尚未完成的工作按[当前队列](review/README.md)推进：先可靠通知后续处理及取消/纠错，再完善运行恢复，再渠道关单/对账和服务生命周期；预发布/实机验收与可选扩展分开，不重复实现已经完成的功能。
...
存量库先备份、停写并核实版本；已有0008未建账本时走adopt-legacy-0008，已有账本用migrate到当前0013。更早版本先按数据库指南处理前置条件，0008会清授权码。当前full_init只接受空库；历史版本曾删表，任何版本都不用于盲目升级。
