...
**本会话固定分支（全仓唯一维护处，TD-277 / V-08）**：`arena/01a0cdc6-codemax-platform`。其他现行文档不再复制这个值：Linux 命令用 `$(git branch --show-current)`，Windows 指南用 `set "BRANCH=..."` 从这一行取值，Agnes 工作流按 `arena/*` 前缀触发。
