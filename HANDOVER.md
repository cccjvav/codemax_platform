...
- **测试基线**：`.venv/bin/python -m pytest -q` → **133 passed**（唯一 warning 是 passlib 的 `crypt` 弃用，无害）
...
**实现层面的取舍（85 条，带编号 TD-xx）集中在 `TECH_DECISIONS.md`**，
...
```bash
.venv/bin/python -m pytest -q                        # 跑测试（当前 133 个，应全绿）
