...
## TD-272：验收前复核批次 1——UI/可访问性机械修（V-04/V-06 一部分）与测试进程 bcrypt 降本（V-03）

2026-09-23。基线 `d61098d`（TD-271）。本批**只做用户已同意的、无新依赖/无新业务规则**的两块：真实浏览器复核列出的排版与可访问性问题，以及测试套件的 bcrypt 热点。商店复购规则（用户已同意改成“只提供下载”）与流程图写入限流属于批次 2/3，本批不动。

**排版与可访问性（V-04/V-06 一部分）**：`base.html` 新增跳过导航、站点图标、`font: inherit`（表单控件默认 13.33px Arial 比正文小一号）、窄屏 16px 输入框（iOS 聚焦缩放）、窄屏导航/页脚 ≥24px 命中区、工具导航一行横向滚动（顶栏 208px → 实测回落）；管理员入口按角色显隐。工具页新增可见页面级 `h2`（值仍只有 `Tool.title` 一处来源）；订单管理页 `h1` 降为 `h2`（整站每页只有一个 `h1`）、窄屏内边距与长订单号换行、只读凭证并入默认展开的 `<details>`、真实发送/停止加红框。`support.css` 时间色 `#64748b → #475569`（4.34:1 → 6.92:1）。`drawio-page.js` 首屏不再重建编辑器 iframe。`mermaid-page.js` 对“未配置模型”给访客文案。

**测试成本（V-03）**：`tests/conftest.py` 在导入应用后把测试进程的 bcrypt 成本降到 4：复核实测 bcrypt 占全量时长 81%（3491 次、1030.9 s / 1276 s），降本后 220 s。生产默认 12 不变，`app/security.py` 不写死轮数（由 `test_auth_crypto.py` 逐条核对）。代价与边界：测试里被哈希的密码强度**只有 4 轮**，任何“抗爆破强度”的实证必须去真环境做，不能引用测试进程的数字；`test_auth_crypto.py` 的两条时序/侧信道断言用 `production_cost` fixture 恢复 12 轮，并且必须同时重建那枚缓存的假哈希 —— 假哈希的轮数在生成时固定，只改 CryptContext 会让“用户不存在”走 4 轮、“密码错”走 12 轮，实测跑出 35.9 倍的**假**差异（夹具缺陷，不是安全回归）。同 fixture 还给 `default_llm` 一个明显的占位 key，让依赖注入之外的用例拿到可断言的上游行为，而不是“未配置 key”这种本地配置错误。回看条件：若 CI 需要“生产成本下跑整个套件”的定期作业，另开一个 nightly job，而不是把主套件改回去。

**未做（如实记录）**：本批没有把任何浏览器断言搬进 CI（沙箱里的 Chromium 装在仓库外，属于一次性证据）；Go/axe 的视觉签收仍在 Windows 指南第 10～12 步。真实浏览器是 131，不是用户机器上的 Edge/Chrome 版本，字体也是 Noto Sans SC 代替微软雅黑。


## TD-273：会话到期与主动退出分开——提示归提示，用户没提交的内容不许丢（复核 N-01）

2026-09-23。TD-270 给所有 401 加了统一处理：清用户快照、通知订阅页、弹登录浮层。提示本身是对的（改前顶栏还显示已登录），但它把「凭证过期」和「用户主动退出」当成同一件事广播，于是 Drawio 页重建编辑器、新 iframe 加载空白图，客服页清空未发送的留言 —— 用户看到「请重新登录后继续；刚才的操作未提交」，同时那份「未提交的操作」已经没了。用同一 Node 桩对比 TD-270 前后的源码：改前不提示但不丢图，改后提示了却丢图。

修法是把原因交给订阅页：`notify(reason)` 的第二参数默认 `"sync"`，`sessionExpired()` 传 `"expired"`；只用一个参数的旧回调不受影响（shop 页就是）。Drawio 在到期时只更新状态文字，不清 identity/xml、不重建 iframe，同一账号重登后可以接着按原 ETag 保存；换账号仍走完整重置（TD-54 的账号隔离不退化）。客服到期时先保留 `#support-body` 再重置视图。

代价与边界：到期后仍未提交的改动**只存在浏览器内存里**，刷新或关闭标签页就会消失（我们没有本地草稿持久化，那是 O-06 的范围，需要单独设计脏稿提示与恢复）；「同一账号」是按下发的 `username` 判断的，用户可能在别处改了密码 —— 那时保存会拿 401/412，页面按既有冲突提示处理，不会静默覆盖。回看条件：做 O-06（脏稿提示/恢复）时把这条内存保留换成显式草稿协议，并补浏览器验收。

## TD-274：已购用户直接去下载、返回商城按订单号恢复状态（V-05，用户确认不复购）

2026-09-24。基线 `f366fc3`（TD-273）。2026-09-23 复核的 N-05 有两个后果：已付款用户回到落地页看到的仍是「立即购买」，点下去会**静默新建一张待支付单**；而收银台点「返回商城」或浏览器后退时，`/shop` 响应带 `no-store`（全局 `SecurityHeadersMiddleware` 对非 `/static/` 一律加，Chromium 实测拒绝入 bfcache），页面是全新加载、`currentNo` 为 null，于是又落回落地页 —— 用户刚付完款却看不到自己的订单。用户 2026-09-24 确认：**单 SKU 数字商品不设计复购**，这是取舍而不是漏功能。

决定的边界：**已购权益**只指 `paid`/`downloaded` 且未全额退款的订单（`refunded` 的单必须回到「立即购买」，否则退完款就再也买不了）；有权益时主按钮换成「已购买，去下载」并给出含订单号的说明，点它 `fetchOrder()` → `render()` 回到那张单，**不发 POST /shop/orders**。恢复顺序**已购优先**：最新一张就是已购单就直接显示该单（刚付完款返回商城走的正是这条路，即使 URL 没带单号）；否则有已购权益就只换按钮，更晚的未付款单不再诱导付款；都没有才恢复**未过期**的待支付单并重新起轮询，过期单不恢复。收银台「返回商城查看订单」链接改为 `/shop?order=<订单号>`，商城页据此先查这一单（正则限定字符集，只接受与订单号同形的参数）。

没有采纳复核 C8 的另一半建议（对 `/shop` 页面放宽 `no-store` 以便 bfcache 生效）：`no-store` 是全站统一策略，为单个页面开例外，将来该页一旦加入任何按身份渲染的内容就会变成缓存泄漏面；脚本恢复已经把「返回后看到该单状态」这件事做实，代价只是多一次网络往返。另外 `?order=` 只是**提示看哪一单**，订单归属仍由服务端按登录用户校验（非本人 404），前端不做任何权益判断。

证据：`tests/test_shop_page.py` 新增 Node 桩组（真跑 `auth.js` + `shop-page.js`，`SHOP_CASE` 决定订单形状）：已购主按钮与「点它不下单、切到该单」在旧代码上先红；`?order=` 的支付成功/已全额退款两种恢复先红；「未过期待支付单恢复且起轮询」先红；两条对照（全额退款后仍可买、过期单不恢复）在旧代码上本来就绿，专门防「恢复过头」。`tests/test_mock_pay.py` 断言返回链接带订单号、无单号时不拼空参数（先红）。沙箱内真实 Chromium 复走一遍：新用户下单 → 模拟收银台 →「返回商城查看订单」→ 直接显示该单支付成功；重开 `/shop` 显示该单；再制造更晚的待支付单后回到落地页、按钮为「已购买，去下载」，点它订单总数不变（2 → 2）；待支付单重新加载后恢复同一单号；手机 390px 无横向溢出。Windows 指南第 12 步的文字同步改成与之一致。

代价与边界：`no-store` 与「不复购」都保持不变，因此返回商城仍是全新加载（一次往返，不是 bfcache 瞬间恢复）；「已购」是前端展示，服务端仍按配额与订单归属独立判定；不提供「再买一份」的入口，这一点如需改变要重新走业务确认。回看条件：如果将来加入多 SKU 或允许复购，`offerDownload()` 与 `restoreLatest()` 的「已购优先」要改成按商品维度判断，而不是按用户最近订单。

## TD-275：流程图写入挂限流桶、注册加每 IP 每日上限（V-02，复核 N-02）

2026-09-24。基线 `f366fc3`。2026-09-23 复核实测：单个 IP 在 15 秒内注册 10 个账号、写入 490 张流程图、约 196 MB 落库，按窗口外推约 1.2 GB/小时/IP；原因不是配额算错，而是**写入路径没有限流依赖** —— `POST/PUT /diagrams` 只能被 2 MiB 的单次请求体预算约束，而账号（每账号 20 MB 字节配额）本身是注册出来的，注册只受 60 秒窗口（10 次/IP）限制。用户 2026-09-24 确认取值按助手的建议来，因此本决策把两个新增配置项的数值一并定下。

**写入桶**：`POST /diagrams` 与 `PUT /diagrams/{id}` 共用 `diagram_write` 桶，配额 `RATE_LIMIT_DIAGRAM_WRITES=60`（每 IP 每 60 秒）。取值理由：前端只在用户点保存或编辑器按 Ctrl+S 时发请求，`refreshList` 也不算写入；即使对着保存键连续点，60 次/分钟仍高于真实操作，同时把突发写入从复核观察到的约 196 MB/15 秒压到每秒 1 次（按同一份 400 KB 的图估算约 24 MB/分钟）。读取（列表、打开、回收站）**不挂桶**：那是正常浏览，限它只会误伤。超限 429 沿用既有文案与 Retry-After。

**注册每日上限**：`RATE_LIMIT_REGISTER_DAILY=20`（每 IP 每 24 小时）。取值理由：账号是字节配额的单位，只限每分钟时一个 IP 一天理论上能开出上万账号；20 个/天对正常用户（含家庭/校园 NAT 共享出口）足够，对批量开号是硬顶 —— 把单 IP 的持续写入从「10 账号/分钟 × 20 MB」压到「20 账号/天 × 20 MB ≈ 400 MB/天」这个量级。429 文案单独写成「该网络今天注册的账号已达上限」，Retry-After 指向最早释放的桶，用户看得出被拒的是哪一类操作。

**实现要点**：每日上限用**独立**的 `Limiter` 实例与 24 小时窗口（`register_daily_limiter` / `REGISTER_DAILY_WINDOW`）。不能塞进主桶表：`admit` 的队头回收假设所有桶共用同一个 window，一条 24 小时的桶会长期挂在队头，让排在它后面、早已过期的短窗口桶收不回来，TD-261 修好的 F-09 容量保护会退化。独立实例里所有桶共用一天窗口，假设重新成立；它被刷满只影响注册，工具/登录/下载不会跟着 429（有回归断言）。`rate_limit` 依赖因此多了三个可选参数（`bucket`/`window`/`quota_detail`），默认行为完全不变。

代价与边界：仍是单进程内存限流（TD-141），多副本各算各的；换 IP 的攻击者仍可绕开单 IP 配额，本批**没有**做复核建议里"可选"的全站总字节软上限与运维告警，也没有把注册失败的 bcrypt 开销前置（O-16）——这两项要动 schema 或额外运维面，等真实部署形态确定再议。IPv6 按 /64 归并，同一出口的多个真实用户共享这两个额度（与 TD-261 的取舍一致）。60 秒窗口仍由 `RATE_LIMIT_WINDOW` 统一控制，调它同时影响所有短窗口 scope；每日窗口是独立常量，不受 `RATE_LIMIT_WINDOW` 影响。回看条件：出现合法的批量注册（例如学校统一开课账号）、单 IP 误伤，或多实例部署需要共享配额时重新评估，不靠调大数值应付。

## TD-276：生产自检文案与行为对齐、gzip 级别 6、静态目录不再公开 Markdown（V-07，复核 C7/O-13/O-14/O-15）

2026-09-24。基线 `0320e68`（TD-275）。本批四项都是复核 P3 项，其中 O-15 用户 2026-09-24 明确"不太清楚，帮用户决定"，因此由助手定稿。

**O-15：保留硬拦，改文案。** 生产启动检查里 `TRUST_PROXY_HEADERS=false` 是硬拒绝启动，旧文案却写「确实不在代理之后才可忽略此项」——无法忽略的东西不能写成可忽略。复核给了两个方向（降级为警告 / 改文案并提供显式开关），我选**保留硬拦 + 改文案**，理由是：在 TLS 终止代理之后关掉它，`_identity` 会把所有用户算成代理这一个 IP，限流从"每 IP 一个桶"退化成"全站一个桶"，这是自助式拒绝服务，比启动失败更糟；而生产里没人会为一条 warning 回头改配置（同类判断见 `check_production_warnings` 的注释：只留给「可能是合法部署」的项）。新文案直接说明为什么直连公网时也可以安全开启：是否采信 `X-Forwarded-*` 另由 `TRUSTED_PROXY_CIDRS` 决定，默认只信任回环地址，公网对端伪造的转发头不会被采信，所以不需要再加一个"我确实直连"的开关（多一个开关就多一种配错）。这条检查**不**验证 CIDR 配得对不对 —— 它只能证明开关打开了，真实代理链路仍归部署签收。

**O-13：gzip level 9 → 6。** starlette 默认 9 级；本机复测最大的分块（647 KiB）level 9 需 23.3 ms、level 6 需 16.4 ms，压缩后同为 138 KiB（复核报告在另一台机器测得 30.7 ms → 22.8 ms、体积差 0.5%）。级别只影响 CPU 与几十字节的差别，不影响正确性；响应头里看不出级别，所以回归直接钉 `GZipMiddleware` 的构造参数，并保留原有的"解压后逐字节相同 / 交付 ZIP 与 Range 不压"用例。

**O-14：静态目录不再公开 Markdown。** `/static/README.md` 曾匿名可读（14 KB，列出全部产物文件名与 SHA 前缀）。文件**不能删**：文档契约要求每个目录都有 README。因此在挂载点用一个 `StaticFiles` 子类把 `.md`/`.markdown`（大小写不敏感）答成 404，其余资源原样透传；没有搬动文件或改文档契约的归属规则。favicon 在 TD-272 已经补上（复核写"缺 favicon"是复核当时的状态），本批只补了"图标仍可访问"的反向断言，防止拦截过宽。

**C7：文档补充 N-07。** 开发模式经 https 代理/隧道访问时，公开基址按请求推导成 `http://…`，与浏览器 `https://` Origin 对不上，`POST /auth/login` 与财务写操作按跨源 403。按复核建议**只改文档**：`docs/DEPLOY.md` 第 3 节说明设置方式，`docs/WINDOWS_LOCAL_RUN.md` 常见问题表加一行症状→处置。生产固定用 `SITE_BASE_URL`，不受影响。
...
## TD-279：ER 图布局重写——按外键深度分列、正交折线不穿表、可读的初始比例（V-04 收尾，复核 N-04）

2026-09-25。基线 `1060c66`（TD-278）。V-04 最后一项：ER 节点文字溢出与连线穿越。

**测量（改前）**：用本项目自己的 16 张表 / 24 个外键（`tests.conftest.project_ddl_for_api`）在真实 Chromium + Noto Sans SC 下渲染，把每条贝塞尔曲线采样后与所有非端点表格求交：**20/24 条线穿过别的表**。原因是布局每行固定 3 张表，线从子表右边中点画到父表左边中点——父表在同一行左边或上一行时，曲线必然横穿中间的表；线还钉在表的竖直中点，看不出是哪一列引用了哪一列。节点固定 230 宽，长列名/注释压出方框。viewBox 永远等于整图，整图 870×2354 被压进画布后文字约为原大小的 40%。

**决定**（仍是纯前端、无新依赖；不引入 dagre/elk 之类布局库）：
- **分列**：按外键深度分列，父表在左、子表在右；环（含 a↔b 互引）用递归栈截断，自引用不参与分列。同层按已放置父表的平均位置排序以减少交叉；单列超过 900px 拆成子列；无任何关系的表单独放最右。
- **宽度与截断**：按文字估算宽度（CJK/全角 1em，宽窄字母分档，只求不低估），夹在 160–340；放不下截断成「…」，原文放进 `<title>` 悬停提示。实测零溢出。
- **正交路由**：线的两端钉在外键列与被引用列那一行；竖线只走列间通道，指向同一父列的线共用一条（总线），跨多列的线走表格上方的车道。因此线段在几何上不可能进入表格——测试逐段核对，而不是抽样。
- **视口**：viewBox 改为画布像素尺寸，缩放交给 d3.zoom。整图按比例放下后仍 ≥ 0.75 就居中显示全图；否则以 0.75 从左上角开始，「查看全图」按钮切换全图/可读尺寸。关系名不再画在线中间（折线中点常落在共用通道里，文字会叠在一起），改为线上的悬停提示；表内外键列标 FK、主键标 PK，箭头指向父表，鼠标停在表上高亮它的全部关系线。

**证据**：同一 DDL 改后实测穿表 0/24、文字溢出 0、axe 零违规、手机 390px 无横向溢出；画布 2361×844（6 层深度，横向展开）。`tests/test_er_page.py` 的几何契约（端点行、线段不入表、表不重叠、宽度/截断、父左子右）与四种构造图（截断、自引用、环、星型拆列走车道）及初始视图用例在旧 `er-layout.js` 上失败、改后通过。

**代价与边界**：原测试里「x1 = 子表右边、y1 = 竖直中点」「每行 3 张」两条坐标断言钉的正是缺陷本身，由更强的几何断言替换（节点顺序、连线集合、关键外键来源等原断言保留）。深 schema 会横向变宽，默认只显示左上角的一部分，需要拖动或点「查看全图」；关系名只能悬停查看，触屏上看不到（表内 FK 标记与箭头仍能读出关系）。宽度是估算不是测量，极端字体下截断可能偏早，不会偏晚。`d3.zoomIdentity` 让 d3 共享分块的导出集合变化，26 个 Mermaid 分块随之出现压缩名重排的产物差异（已核实旧源码构建零漂移）。

## TD-280：修改密码入口、Mermaid 按需加载与 Vite 预加载地址（V-06 收尾）

2026-09-25。基线 `84866da`（TD-279）。

**问题**：
- `POST /auth/password`（TD-70）早就实现，但整站没有入口。Windows 指南让管理员去 Swagger `/docs` 改密，而生产环境关闭 `/docs`——用户怀疑密码泄露时无法自助改密。
- Mermaid 页首屏静态下载 16 个文件、约 669 KiB，只看一眼页面、或站点没配模型（生成必然失败）的访客也要付这笔流量。
- 构建时发现：Vite 给动态 import 生成的预加载地址是 `"/" + 文件名`，而产物挂在 `/static/js/`，Mermaid 内部按图类型加载分块时一直在请求 `/xxx.js` 这类 404（JS 预加载失败不报错，功能正常但预加载无效）。

**决定**：
- **入口放在顶栏用户名上**，而不是新增按钮：TD-278 把手机顶栏压到两行 88px，第一行只放得下站点名 + 两个控件。用户名改为 ghost 按钮（可访问名称「用户名（修改密码）」），打开与登录浮层同构的对话框：原密码、新密码两次、隐藏的 username 字段（密码管理器据此把新密码存到对的账号）。两次不一致与新旧相同在前端先挡（不消耗改密限流额度），401 交给统一的会话到期处理；成功后本页保持登录（服务端已换新 cookie），提示其他设备需要重新登录。
- **Mermaid 按需 import**，在提交时与模型请求并行下载；下载失败不缓存 promise，下次点击会重试；渲染器下载失败时仍显示生成的源码。`securityLevel: "strict"` 不变。
- **`vite.config.mjs` 设 `base: "/static/js/"`**，预加载地址与实际挂载位置一致。

**证据**：真实 Chromium（Noto Sans SC）——改密：两次不一致/原密码错提示正确；成功后另一个会话 `/auth/me` 200 → 401、本页 200；Esc 焦点回用户名按钮；桌面与 390px 手机 axe 零违规，顶栏仍 88px、站点名不截断。Mermaid：提交前只下载 `auth.js` + `mermaid-page.js`（3.5 KiB），提交后 19 个分块全部来自 `/static/js/`、零 404；桩掉模型回复后类图在点击后约 0.2 秒渲染出来。新增的 Node/产物用例在改前的源码上失败。
