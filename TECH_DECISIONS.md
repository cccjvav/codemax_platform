...
## TD-259：首发定位为演示/免费工具，付费准入项延后而不删除

2026-09-19。用户确认首发类型为**演示 / 免费工具**：`SHOP_PAY_MODE=mock`，所有资金类开关（WX_* 真实商户、`WX_BILL_READ_ENABLED`、退款发送/核验监督等）保持关闭；ROADMAP 中 L-00（主体/渠道准入）、L-02（真实商户联调）等只在决定收费前才成为阻断项，不再作为当前批次的前置等待。工程优先级改为公开接口边界（G1：A-01 请求体预算 → A-05 embedding 索引严格整型 → A-02 LLM 客户端流式/上限/超时 → A-03 下单限流与在途租约 → A-04 登录来源校验），因为演示站同样暴露匿名可达的解析、LLM 与下单入口。第十八批先做零风险文本修正（过期测试文档字符串、手写数量、失效注释、断言消息、README API 摘录）并把独立全面检查报告纳入仓库与文档站。

放弃方案：把收费准入当作当前阻断继续等待外部材料；或因免费首发而删除支付代码路径。代价：付费链路代码仍要随重构同步维护与测试，文档中"可收费"表述必须继续标注未验收；本会话在固定分支 `arena/01a0ba44-codemax-platform`（自 `ea11619` 分出）工作，合回 `arena/01a08bf5-codemax-platform`/`main` 只能经 PR 并取得用户明确授权。何时回头改：计划开始收费时先执行 L-00/L-02，再评估 P2-1..P2-3、P2-6、P2-7 与迁移 0018。

## TD-260：请求体预算放在最内层纯 ASGI 中间件，按精确路径分档，不靠 Pydantic 或代理

2026-09-20（A-01，审计 F-01 / 报告 P2-9）。FastAPI 先 `await request.body()` 再校验，`max_length` 对内存无约束，默认 422 还回显 `input`。修复放在 `app/middleware.py::RequestBodyBudgetMiddleware`（最先 add、最内层，413/408 也经过安全头与日志），规则：`Content-Length` 声明超预算立即 413、一个字节不读，非法值 400；无长度或伪造长度时包装 `receive` 逐块计数，越界由中间件自己发 413 并向应用返回 `http.disconnect`、丢弃应用迟到的响应——若让 FastAPI 先应答，它会把 `ClientDisconnect` 包装成 400 "error parsing the body"，状态码就错了；从收到请求头起读体总时限 `REQUEST_BODY_TIMEOUT_SECONDS`（默认 60 秒，`asyncio.timeout_at` 包每次 receive），超时 408。拒绝响应带 `Connection: close`，h11 应答后关闭连接而不是继续吞剩余请求体。预算按**精确正则**分三档：默认 64 KiB；`/tools/er-diagram`、`/tools/word-export` 128 KiB（DDL 20000 字符 × 6 字节转义）；`/diagrams`、`/diagrams/{id}` 4 MiB（content 500000 字符）；`/shop/pay/notify`、`/shop/refunds/notify` 固定 2×64 KiB 兜底，保留处理函数在 64 KiB 处的微信格式 `{"code":"FAIL"}` 413。`tests/test_request_body_budget.py` 用 schema 的 `MaxLen` 核对每档能装下最大合法体（含 JSON `\uXXXX` 最坏情况）。422 处理器只返回 loc/msg/type。

放弃方案：只调 Pydantic 上限（读完才校验）；只靠反向代理 `client_max_body_size`（直连端口与内网调用不经代理，且代理上限低于 4 MiB 会误伤 drawio）；Starlette `BaseHTTPMiddleware`（先读整个体，破坏流式与回调原始字节）；uvicorn 参数（没有请求体大小/读体时限选项，h11 只做帧校验）。代价：新增四个配置项，改小前必须对照 `app/schemas.py`；边读边发的流式路由若在应答开始后超限只能停止喂数据，不能改状态码（当前没有这类路由）；伪造小 `Content-Length` 在真实 h11 下先被帧校验拒为 400，中间件计数是对其它 ASGI 服务器/传输的纵深防御。本地 uvicorn 实测：50 MB 声明 → 413、0 字节；50 MB 分块 → 413 且连接关闭（curl 仅把约 1.8 MB 写进 socket 缓冲）；2 秒时限下慢体 → 408 并关闭；受控变异（拆中间件注册 / 摘 422 处理器 / 默认档放大到 32 MiB / 去掉 `Connection: close`）分别使 7 / 3 / 4 / 3 条回归转红，合计 12 条不同用例，源码已恢复并核对 SHA。这不是代理层或生产网络验收。何时回头改：新增文件上传/多部分表单或 SSE 等边读边发路由时，按路由声明预算并处理应答开始后的超限。

## TD-261：embedding index 用 `type(...) is int` 严格校验，不接受 bool/float

2026-09-20（A-05，审计 F-05 / 报告 P3-27）。OpenAI 兼容的 `/embeddings` 只约定每条带 `index`，不保证数组有序；`LLMClient.embeddings` 因此按 `index` 排序再与输入对齐。旧实现只做「排序后逐位相等」的值比较，而 `False == 0`、`0.0 == 0`：JSON 里的 `false`/`0.0` 会被当作合法下标接受，同批里再出现别的不整型 index 时，向量与语料的对应关系可能静默错位——不抛异常，只是永远检索错（最坏的一类 bug）。现在排序前显式要求 `type(row["index"]) is int`（bool 是 int 子类，用 isinstance 会放行），失败转 `LLMError("…embedding index 必须是整数…")`；缺失（KeyError）、重复/越界（排序后集合比较）保持原有 `LLMError`。保护性回归在 `tests/test_faq_semantic.py`：bool/float 四例在旧实现下全部被接受，字符串/None/容器与缺失/重复/越界锁定行为。

放弃方案：`isinstance(index, int)`（放行 bool）；`int(index)` 强制转换（把 `"0"`、`0.0` 悄悄变成合法下标，比拒绝更危险）；只信任数组顺序不做校验收口（供应商明确不保证有序）。代价：合法的非整型数值下标（如 `1.0`）会被拒绝——按 OpenAI 兼容协议 index 是整数，这是有意收紧；若将来遇到只返回浮点下标的提供方，应在适配层转换并在此登记，而不是放宽客户端。受控变异把 `type(...) is int` 改回 `isinstance(..., int)` 后 bool 两例转红，源码已恢复并核对 SHA。F-05 诊断探针退役（`tests/audit_handoff_probes.py` 现余四项）。何时回头改：接入只返回浮点/字符串 index 的提供方，或协议明确允许非整数下标时。
