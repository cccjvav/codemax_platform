...
| TD-183 | `test_faq_p95_survives_concurrent_heaviest_endpoint` 用**绝对阈值** `p95 < 80 ms`，在负载高的机器上会随机变红 | 改成相对比值（TD-161 的主张） | 这条是 TD-161 里刻意保留的**唯一例外**，理由是 80 ms 是 ROADMAP 写死的需求而非本机测量值。实测代价：在本沙箱跑真 PostgreSQL 时它会随机失败 —— 对父提交 `e129eac` 连跑 5 次 **3 次红**（p95 81.8 / 通过 / 123.0 ms），对含 TD-65 的树连跑 3 次 **3 次红**（112.8 / 89.4 / 85.7 ms）。**两棵树数值区间重叠，说明这是既有的环境敏感性，不是 TD-65 引入的**（已用 `git worktree` 在同一台机器上 A/B 对比确认）。CI 上稳定通过（本分支此前 60 次运行全绿）。**后续又观测到两次，且换了测试与后端**：含 TD-78 的树在 SQLite 上跑全量时 `test_support_endpoint_p95_under_concurrency` 变红、同一棵树在真 PostgreSQL 上 382 条全绿；而上一轮恰好相反（真 PG 红、SQLite 绿）。抖动在两个后端与多条绝对阈值用例之间游走 —— 这更像是**阈值写法的问题**，而不是某一处真的性能退化 | 若 CI 机器也开始随机红，就按 TD-161 改成「并发时 p95 ≤ 空载 p95 的 N 倍」，把 80 ms 留在文档里当需求 |
