...
| TD-213 | 依赖漏洞：`python-jose` 升到 3.5.0，其余 15 条已知 CVE **显式挂账不修**，并在 CI 加 `pip-audit` | 放弃了「依赖版本钉死就不管了」，也放弃了「一次把所有依赖升到最新」 | 复审的 P1-7 只点了 `python-jose==3.3.0` 两条 CVE。装上 `pip-audit` 实跑才发现**真实面比这大得多**：6 个包共 15 条唯一漏洞（`starlette` 7 条、`python-multipart` 4 条、`fastapi`/`python-dotenv`/`pytest`/`ecdsa` 各 1 条）。<br>**已修**：`python-jose` 3.3.0 → **3.5.0**，堵住 CVE-2024-33663（OpenSSH ECDSA 算法混淆，可用公钥签名）与 CVE-2024-33664（JWE 解压 DoS）—— 本站用它签发登录令牌，算法混淆正好打在核心用途上。升级后 JWT 相关 58 条测试全绿。<br>**不修的理由**：`starlette` 的修复版本要 0.40/0.47/1.0/1.1/1.3，而 starlette 由 fastapi 钉住 —— 要修完得把 fastapi 从 0.104.1 跳到 0.141.x（`requires starlette>=0.46`），**跨两个大版本**；`pytest` 7.4.4 → 9.x 同样跨两个大版本且**仅测试依赖、不进生产镜像**；`ecdsa` **上游至今没有修复版本**。<br>**为什么仍然加 audit job**：`requirements.txt` 是手工钉版本的，钉死之后没有任何机制会在上游披露新 CVE 时通知我们 —— 这两条 python-jose 的 CVE 就在文件里躺了很久没人知道。job 带 15 个 `--ignore-vuln`（逐条列在 ci.yml 注释里）以保持绿色，从而能抓住**新**披露的漏洞；**债被挂账，没有被隐藏**。`--strict` 保证「扫描器没报错」和「扫描器没扫到」能区分开。<br>另在 `test_ops.py` 钉了一道本地防线：`python-jose` 低于 3.4.0 直接测试失败（CI 可能被跳过，这条跟着每次 pytest 跑）。 | 那 15 条仍是真实存量风险。清账路径是一次独立的依赖升级：fastapi 0.104.1 → 0.141.x 带动 starlette 到 1.x，需要完整回归（尤其 `BaseHTTPMiddleware` 行为与 CSP 相关中间件）；`ecdsa` 只能等上游或换 JOSE 库 |
