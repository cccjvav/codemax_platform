...
`https://你的正式域名/shop/refunds/notify`与收款WX_NOTIFY_URL分开；无新环境变量/数据库迁移，仍部署0011。只有原渠道办理流程真正指定了该notify_url才会收到对应通知，不能改收款地址来假装接通退款，不能承诺补发旧通知。可信商户/app/APIv3密钥和平台证书/显式公钥ID继续复用，不从入站请求下载密钥。
