...
订单使用内容寻址快照；storage下`.snapshots`必须随数据库备份，不能作为缓存清理。新商品文件需先准备、不得超过512MiB；Windows实际硬链接支持需本机检查。旧库运行0010迁移后，历史交付按原合同由管理员显式绑定；不改用当前商品、不重跑full_init。人工确认需真实参考号/整数分金额/依据，见[第三批](../review/RELEASE_BLOCKERS_PHASE3.md)。SQLite测试现在是独立连接的临时文件，仍不代表生产PG或本机商户验收。
