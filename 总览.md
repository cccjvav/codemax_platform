...
| 文件 | 职责 | 关键事实 |
| --- | --- | --- |
| **`requirements.txt`** | **24 个依赖，全部钉死版本** | 分四段：运行时 18 / 测试 4 / 文档工具 1 / 静态检查 1。**`requirements-dev.txt` 不存在** —— 一条命令装齐（取舍见 TD-202） |
...
```bash
# ── Install ────────────────────────────────────────────────
pip install -r requirements.txt          # 24 个依赖，含测试、lint 与文档工具
