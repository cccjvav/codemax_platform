...
| 技术 | 版本 | 用途 |
| --- | --- | --- |
| **pytest** | 7.4.4 | 测试框架（35 个测试文件 / 530 个用例） |
...
| **测试层** | `tests/` | 37 个 py | 536 个用例，跑 SQLite 与真 PG 两种后端 |
...
| `tests/` | **测试层**：35 个测试文件 / 536 个用例，跑 SQLite 与真 PostgreSQL 两种后端 | [查看详情](./tests/README.md) |
| `scripts/` | **工具脚本**：建表脚本深度体检（PGlite）+ 文档站构建（2 个文件 / 966 行） | [查看详情](./scripts/README.md) |
| `.github/workflows/` | **CI**：3 个 job（lint / SQLite / 真 PG），失败时把 pytest 输出发成 PR 评论（229 行） | [查看详情](./.github/workflows/README.md) |
