...
| `drawio.html`（最复杂前端） | postMessage + 乐观锁 + 登录态 | [查看详情](./app/templates/README.md#-文件名drawiohtml221-行-最复杂) |
| `full_init.sql`（7 表） | 建表脚本，CI 连跑两遍验幂等 | [查看详情](<./database init/README.md#-文件名full_initsql115-行>) |
...
| 文件 | 职责 | 关键事实 |
| --- | --- | --- |
| **`requirements.txt`** | **22 个依赖，全部钉死版本** | 分三段：运行时 16 / 测试 5 / 静态检查 1。**`requirements-dev.txt` 不存在** —— 一条命令装齐 |
...
```bash
# ── Install ────────────────────────────────────────────────
pip install -r requirements.txt          # 22 个依赖，含测试与 lint
...
cd "database init"
python db_init.py                        # 幂等：自动建库 + 建表，测试账号 admin/123456
cd ..
...
| `scripts/` | **运维脚本**：用 WASM 版真 PostgreSQL（PGlite）体检建表脚本（68 行） | [查看详情](./scripts/README.md) |
