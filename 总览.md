...
| `Settings`（41 项配置） | 全部环境变量的唯一真相 | [查看详情](./app/README.md#-文件名configpy84-行) |
...
| LLM 调用封装 | 超时/预算/异常映射 | [查看详情](./app/tools/README.md#-文件名llmpy146-行) |
| FAQ 融合召回 | BM25 + 向量，权重 0.6 | [查看详情](./app/tools/README.md#-文件名faqpy355-行) |
| 意图路由 | 规则层，零成本兜底 | [查看详情](./app/tools/README.md#-文件名intentpy182-行) |
| 客服三层编排 | 规则→FAQ→RAG→转人工 | [查看详情](./app/tools/README.md#-文件名supportpy270-行) |
...
| CI 四个 job | lint / docs / SQLite / 真 PG | [查看详情](./.github/workflows/README.md#-文件名ciyml229-行) |
---
...
| 技术 | 版本（实测） | 用途 |
| --- | --- | --- |
| **FastAPI** | 0.141.1 | Web 框架，全部 HTTP 接口（⚠️ 0.141 起 `app.routes` 结构变了，见 TD-213） |
...
| **Pydantic** | 2.13.5 | 数据校验与配置 |
| **pydantic-settings** | 2.15.0 | `.env` 配置加载 |
| **python-jose** | 3.5.0 | JWT 签发与校验（3.3.0 带 CVE-2024-33663/33664，见 TD-213） |
...
- 自研 JS 源码在 `app/frontend/`（TD-222）：`er-layout.js` + `er-page.js`（ER 图绘制）与
  `app/frontend/auth.js`（137 行，全站共享登录态模块，S2-02-2）；
  另有 `pay_qr.svg`（manual 支付模式的占位收款码）
...
| **GitHub Actions** | CI（4 个 job：lint / docs / SQLite / 真 PostgreSQL） |

### 2.5 测试与质量

| 技术 | 版本 | 用途 |
| --- | --- | --- |
| **pytest** | 9.0.3 | 测试框架（41 个测试文件 / 628 个用例） |
| **pytest-asyncio** | 1.4.0 | 异步测试（`asyncio_mode = auto`） |
...
```text
                    浏览器
                      │
   ┌──────────────────┴──────────────────┐
   │  模板层  app/templates/   8 个 HTML   │  Jinja2 SSR：HTML 外壳 + TDK
   │  静态层  app/static/      2 JS + 1 SVG │  er.js / auth.js / pay_qr.svg
   └──────────────────┬──────────────────┘
                      │
   ┌──────────────────┴──────────────────┐
   ...
   横切：app/config.py（41 项配置）、security.py、ratelimit.py、
        cpu_pool.py（重活走进程池，不阻塞事件循环）、middleware.py
```

| 层 | 目录 | 规模 | 职责 |
| --- | --- | --- | --- |
| **模板层** | `app/templates/` | 8 个 HTML | SSR 出页面外壳与 TDK；图表交给客户端渲染 |
...
| **测试层** | `tests/` | 38 个 py | 565 个用例，跑 SQLite 与真 PG 两种后端 |
...
浏览器 er.html
  ...
  │                                     └─ 返回 {tables, edges}
  │     ◄── JSON ──────────────────────────────────────────────────────
  └─ renderEr("#er-canvas", data)   app/frontend/er-page.js
...
| `.env.example` | **41 项配置模板** | 与 `app/config.py` 的 `Settings` **1:1 对应**（对称差为空） |
...
cp .env.example .env                     # 然后填 DB_PASSWORD / LLM_API_KEY 等 41 项
...
| `app/`（根） | **横切基础设施**：配置、ORM 模型、安全、限流、进程池、中间件、站点清单、微信支付、存储、订单状态机（16 个 py / 1 440 行） | [查看详情](./app/README.md) |
| `app/templates/` | **模板层**：8 个 Jinja2 模板，SSR 出外壳与 TDK，图表客户端渲染（820 行） | [查看详情](./app/templates/README.md) |
| `app/static/` | **静态资源层**：`er.js`（ER 图渲染，`layoutEr` 可被 node require）、`auth.js`（全站登录态）、`pay_qr.svg`（占位收款码），共 286 行 | [查看详情](./app/static/README.md) |
| `database init/` | **数据层脚本**：建库建表（7 表 / 4 外键）+ 5 个迁移脚本（8 个文件 / 521 行） | [查看详情](<./database init/README.md>) |
| `tests/` | **测试层**：36 个测试文件 / 565 个用例，跑 SQLite 与真 PostgreSQL 两种后端 | [查看详情](./tests/README.md) |
| `scripts/` | **工具脚本**：建表脚本深度体检（PGlite）+ 文档站构建（2 个文件 / 1 003 行） | [查看详情](./scripts/README.md) |
| `.github/workflows/` | **CI**：4 个 job（lint / docs / SQLite / 真 PG），失败时把 pytest 输出发成 PR 评论（229 行） | [查看详情](./.github/workflows/README.md) |
| 根目录 | **入口与构建配置**：`main.py`、`ruff.toml`、`requirements.txt`、`Dockerfile`、`docker-compose.yml`、`pytest.ini`（153 行） | [查看详情](./docs/ROOT_FILES.md) |
...
| [TECH_DECISIONS.md](./TECH_DECISIONS.md) | 取舍台账（174 个 TD，编号至 TD-226） |
