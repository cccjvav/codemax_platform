...
| ID / 优先级 | 工作与依据 | 验收标准 | 环境/约束 |
| --- | --- | --- | --- |
| V-01 / P2（回归）——**已完成（2026-09-23，TD-273）**：`notify(reason)` 区分 `expired` 与主动退出；Drawio 到期不重建编辑器、同账号重登可继续保存，客服草稿保留 | N-01：会话过期时保留 Drawio 未保存的图与客服草稿（TD-270 的 `sessionExpired` 触发登出通知，`syncAuth(null)` 重置编辑器、`onUser(null)` 清空草稿） | Node 桩：过期→同账号重登，编辑器内容与草稿仍在；过期→换账号仍清空（账号隔离不退化）；新断言在当前代码上先红 | 前端 3 文件 + 产物；无依赖/配置 |
| V-02 / P2 ——**已完成（2026-09-24，TD-275）**：`POST/PUT /diagrams` 共用 `diagram_write` 桶（`RATE_LIMIT_DIAGRAM_WRITES=60`，读取不挂桶）；注册加每 IP 24 小时上限 `RATE_LIMIT_REGISTER_DAILY=20`（独立桶表，避免拖累主桶表的过期桶回收） | N-02：流程图写入无每 IP/全站上限（单 IP 15 秒约 196 MB） | 回归 `tests/test_ratelimit.py` 新增 5 条（旧路由上先红）：POST/PUT 共用桶且第三次 429、读取不受影响；注册第三次 429 且文案/Retry-After 指向每日；每日桶用尽后工具接口仍 200；两条换 IP/同 /64 的身份用例 | 取值按助手建议（用户 2026-09-24 确认）；单进程内存限流，不做全站字节软上限与告警（复核列为可选） |
...
| V-04 / P3（UI）——**完成（2026-09-23，TD-272；2026-09-25 续，TD-278；2026-09-25 收尾，TD-279）**：TD-279 ER 布局重写 —— 按外键深度分列、宽度按文字估算并截断、正交折线只走通道与车道（本项目 16 表实测穿表 20/24 → 0、文字溢出 0），初始比例 0.75 + 「查看全图」。TD-278 真实浏览器复核后修正 —— 手机顶栏两行 grid（189px → 88px）、手机 16px 输入框规则移到样式表末尾真正生效（此前被覆盖，实测 13px）、付款后状态区先于「我的订单」、Drawio 已登录隐藏「需登录」提示、订单管理页空状态/选单前占位（axe `list` 清零）、收银台单 h1。TD-272：跳过导航、`font: inherit`、窄屏 16px 输入框、≥24px 命中区、窄屏顶栏收窄、管理员入口按角色显隐、工具页可见页面标题、管理页单 h1/长单号换行/只读分区/危险红框、客服时间色 6.92:1 | 手机订单管理页横向溢出 28px；ER 节点文字溢出/连线穿越与遮挡；按钮不继承字体（13.33px Arial）、iOS 输入框 <16px 缩放；客服时间 4.34:1；Drawio 下拉框无可访问名称；工具页无页面标题、管理页双 h1；手机顶栏 208px | 浏览器/Node 断言：390px 无横向滚动、ER 文字宽 ≤ 节点宽、axe-core 0 违规、按钮/输入框字号继承 | CSS + er-layout + 产物；真实浏览器验收仍归 L-04 |
| V-05 / P3（流程）——**已完成（2026-09-24，TD-274）**：已购用户主按钮「已购买，去下载」且点它不再新建订单；收银台返回链接带 `?order=` 直接恢复该单状态；加载/登录后自动恢复一次（已购优先、未过期待支付单接着付、过期单不恢复）；Windows 指南第 12 步文字同步 | 单标签页“返回商城”落回落地页（`/shop` no-store 使 bfcache 失效，`currentNo` 丢失）；已付用户再点购买静默新建订单 | 返回后显示该单状态（`/shop?order=` 或登录后自动恢复最近订单）；已购用户主按钮为“去下载”；Windows 指南第 12 步文字同步 | 复购规则属业务决定（用户 2026-09-24 确认：不设计复购，全额退款后可再买） |
