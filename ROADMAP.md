...
| V-05 / P3（流程）——已购用户「已购买，去下载」与返回商城恢复订单状态待实施（用户已确认：不按复购设计） | 单标签页“返回商城”落回落地页（`/shop` no-store 使 bfcache 失效，`currentNo` 丢失）；已付用户再点购买静默新建订单 | 返回后显示该单状态（`/shop?order=` 或登录后自动恢复最近订单）；已购用户主按钮为“去下载”；Windows 指南第 12 步文字同步 | 复购规则属业务决定 |
