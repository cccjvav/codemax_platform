...
应在 `HANDOVER.md` 首段「本会话固定分支」写的那个分支（形如 `arena/<会话编号>-codemax-platform`；本指南不复制这个值）。如果有修改或在其他分支，先保留原目录，不执行 reset/clean/强制切分支；用下面的方式克隆到一个**尚不存在**的新目录，并把后文路径一起替换。

**尚未克隆**时，在终端 A：

```cmd
if not exist C:\work mkdir C:\work
set "BRANCH=arena/<HANDOVER 里写的本会话分支>"
git clone -c core.autocrlf=false --branch %BRANCH% --single-branch https://github.com/cccjvav/codemax_platform.git C:\work\codemax_platform
cd /d C:\work\codemax_platform
```
...
1. 点右上角自己的用户名（`owner`，带边框的按钮），弹出「修改密码」。
2. 填原密码，新密码输两次（6–64 字），点「确认修改」。
3. 期望出现绿色提示「密码已修改。其他设备和浏览器上的登录已失效……本页保持登录」，按钮变成「关闭」。两次新密码不一致或原密码错，会在浮层里显示红字，不会改。
4. 点「关闭」，回首页退出，用新密码重新登录。

改密走的是同源 Cookie，本页不会掉线；已在其他浏览器登录的同一账号会在下次请求时要求重新登录（这正是改密的用途之一）。遇到「登录已过期」先重新登录再改；不要关闭后端鉴权。生产环境不开放 `/docs`，所以这个浮层就是正式的改密入口（TD-280）。
...
3. 点「模拟支付成功」，页面下方出现「返回商城查看订单」；点它回到商城。链接会带上订单号（`/shop?order=…`），商城页直接显示这一单的状态，应从“待支付”变为“支付成功”。浏览器后退或重新打开 `/shop` 也会自动恢复最近一张订单（页面带 `no-store`，不能靠 bfcache 恢复）。再点「查看 / 刷新我的订单」也能选到该单。
4. 点「下载交付物」。应下载 ZIP，打开其中 README.txt，内容为 `CodeMax acceptance test file`。
5. 回 `/shop`：主按钮应显示「已购买，去下载」（已购用户不再看到「立即购买」，同一商品无需重复购买），点它直接回到该订单；再点下载应该还能下载 —— 已领过链接不等于永久消耗权益。
...
```cmd
rem 分支名看 HANDOVER.md 首段；每个新开的 CMD 窗口都要重新 set
set "BRANCH=arena/<HANDOVER 里写的本会话分支>"
git clone -c core.autocrlf=false --branch %BRANCH% --single-branch https://github.com/cccjvav/codemax_platform.git C:\work\codemax_acceptance
```
