...
已有本项目旧库不是新库练习：先停止服务、备份并核对恢复，再按数据库指南运行migrate到0010。历史订单会要求管理员核实原交易和原文件后绑定，不会自动用今天的商品替代。人工收款参数已收紧，详细JSON、只读ledger和旧单核准见[第三批说明](review/RELEASE_BLOCKERS_PHASE3.md)。先继续使用本指南的隔离mock验收，别用真实付款测试这些步骤。
